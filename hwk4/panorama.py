...
def harris_corners(img, window_size=3, k=0.04):
    ...
    #corner response at each pixel
    detM = np.multiply(A,B) - np.multiply(C,C)
    traceM = A+B
    response =detM -k *(traceM**2)#math equation
...
def simple_descriptor(patch):
    ...
    mean =np.mean(patch)    #mean
    std_dev = np.std(patch) #standard deviation

    normalised_patch = (patch - mean)/std_dev

    if std_dev ==0:
        std_dev =1
        
    feature = normalised_patch.reshape(-1)
...
def match_descriptors(desc1, desc2, threshold=0.5):#
    ...
    N = desc1.shape[0]
    dists = cdist(desc1, desc2) #euclidean distance 

    ### YOUR CODE HERE
    pass
    #Sort the dist from smallest to largest and return N * N    
    for i in range(N):
        dist = dists[i, :]
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    #Compute inliers
        ...
        temp_inliers = current_inliers.sum()

    #Keep the largest set of inliers
        if temp_inliers > n_inliers:
            max_inliers = current_inliers
            n_inliers = temp_inliers
        ...
        for j in range(cols):
            for m in range(G_cells.shape[2]):
                for n in range(G_cells.shape[3]):
                    idx=int((theta_cells[i,j,m,n] //degrees_per_bin)%n_bins)
                    cells[i,j,idx]+=G_cells[i,j,m,n]
                    
    cells = (cells - np.mean(cells))/ np.sum(cells)
    block = cells.flatten()
    ### YOUR CODE HERE
...
def linear_blend(img1_warped, img2_warped):
    ...
    ### YOUR CODE HERE
    pass
    left_space = np.array(img1_mask, dtype = np.float64)
    right_space = np.array(img2_mask, dtype= np.float64)

    #Define a weight matrices for img1_warped and img2_warped
    left_space[:,left_margin: right_margin]= np.tile(np.linspace(1,0, right_margin - left_margin), (out_H,1))
    right_space[:, left_margin: right_margin] = np.tile(np.linspace(0,1, right_margin - left_margin), (out_H,1))
    
    #Apply the weight matrices to their corresponding images
    #and combine images
    merged= (left_space * img1_warped) + (right_space * img2_warped)
